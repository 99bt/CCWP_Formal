...
class ccwp_test1(unittest.TestCase):
    ...
    def test01_Editaddress(self):
        ...
        try:
            ...
            self.Address.book_sender()

        except Exception as e:
            logger.critical("----------增删查改地址执行异常--------异常原因:%s" % e)
    ...
    def test02_sendOrder(self):
        try:
            ...
            # self.assertIn(get_time,date_time)
            self.assertEqual(receiver_name2, sender_nameNO)
            self.assertEqual(receiver_detail2, sender_detailNO)
            self.assertEqual(sender_name2, receiver_nameNO)
            self.assertEqual(sender_detail2, receiver_detailNO)
            ...
            logger.critical("----------实时单*下单执行异常--------异常原因:%s" % e)

            t = self.driver.get_screenshot_as_file(screenshot)
            print("异常截图结果：", t)

            for i in phone_numbers:
                templateparam = {"time": str(time.strftime("%Y-%m-%d %H:%M:%S")), "business": "【实时单寄件执行】","Other": "系统异常情况！！！"}
                sms_send.send_sms(str(i), str(templateparam))
    ...
    def test06_checkPieces(self):
        try:
            ...
            if text:
                ...
                self.assertEqual(self.Check.get_inputBno(),"803351784837")#803351784837
